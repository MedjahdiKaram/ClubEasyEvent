...
var messages = (window.CEE_Public && CEE_Public.i18n) ? CEE_Public.i18n : {};

function getMessage(key, fallback) {
    return (messages && Object.prototype.hasOwnProperty.call(messages, key) && messages[key]) ? messages[key] : fallback;
}
...
$(document).on('click', '.cee-rsvp-button', function(){
    ...
    $container.find('.cee-rsvp-status').text(getMessage('loading', '…'));
    ...
    $.ajax({
        ...
        if (resp && resp.success) {
            $container.find('.cee-rsvp-status').text(getMessage('success', ''));
        } else {
            $container.find('.cee-rsvp-status').text(getMessage('error', ''));
